...
if OPUS_RUNTIME not in sys.path:
    sys.path.insert(0, OPUS_RUNTIME)
...
anthropic_text = provider_module("anthropic_text")
openai_text = provider_module("openai_text")
deepseek_text = provider_module("deepseek_text")
groq_text = provider_module("groq_text")
fireworks_text = provider_module("fireworks_text")
venice_uncensored_text = provider_module(
    "venice_uncensored_text"
)
elevenlabs_tts = provider_module("elevenlabs_tts")
openai_tts = provider_module("openai_tts")


# Use Savant's existing environment loader.
# This loads credentials without displaying them.
openai_tts.load_environment()


TEXT_REQUEST = {
    "message": "Reply with exactly: savant-provider-live",
    "system": (
...
def run_probe(
    *,
    ...
    try:
        ...
        result["response"] = clean_error(exc)
        return result

    if not result["configured"]:
        result["status"] = "not-configured"
        result["response"] = (
            "required provider configuration unavailable"
        )
        return result
    ...
    try:
        ...
        returned_text = preview(
            response.get("text", "")
        )

        if response.get("ok") and returned_text:
            result["status"] = "ok"
...
def main() -> int:
    ...
    print()
    print("savant // opus provider live verification")
    print("=" * 76)
    print(
        f"{'provider':<16}"
        f"{'status':<18}"
    ...
    print("summary")
    print(
        f"working: {len(successful)}/{len(results)}"
    )
