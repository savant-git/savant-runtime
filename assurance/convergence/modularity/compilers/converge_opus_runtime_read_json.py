...
schema = (
    "savant.assurance."
    "converge-opus-runtime-read-json.v1"
)
...
shared_path = runtime_root / "json_io.py"

targets = (
    runtime_root / "router.py",
    runtime_root / "voice_orchestrator.py",
)
...
expected_digest = (
    "dda8dda05106ab7481fd27e19cd28ddd"
    "b65368bbaf146bcf302cddff571d4c42"
)
...
def verify_shared() -> None:
    ...
    if node is None:
        raise ConvergenceError(
            "shared read_json primitive missing"
        )
    ...
    if actual != expected_digest:
        raise ConvergenceError(
            "shared read_json substance does not "
...
def remove_local_function(
    ...
    if actual != expected_digest:
        raise ConvergenceError(
            f"{path}: read_json substance changed; "
...
def validate(
    ...
    if shared_import not in text:
        raise ConvergenceError(
            f"{path}: shared read_json import missing"
        )
    ...
    if find_function(
        ...
        raise ConvergenceError(
            f"{path}: duplicated local read_json remains"
        )
