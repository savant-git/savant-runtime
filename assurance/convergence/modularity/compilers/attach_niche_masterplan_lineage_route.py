...
schema_version = (
    "savant.assurance."
    "attach-niche-masterplan-lineage-route.v1"
)
...
import_statement = (
    "from masterplan_lineage_projection "
    "import lineage_projection"
)

route_anchor = '''        if path == "/api/masterplan/index":
            self.send_json(
                index_projection()
            )
            return
...
route_source = '''        if path == "/api/masterplan/lineage":
            self.send_json(
                lineage_projection()
            )
            return
