...
schema = "savant.assurance.modular-primitive-convergence.v2"
...
historical_path_sequences = (
    ("evolution", "structure-migration", "baseline-captures"),
    ("runtime", "identity-promotion", "transactions"),
)
...
def normalized_constant(value: Any) -> Any:
    ...
    if isinstance(value, complex):
        return {
            ...
            "real": normalized_constant(value.real),
            "imag": normalized_constant(value.imag),
        }
...
def excluded_directory(part: str) -> bool:
    ...
    return any(
        ...
        for prefix
        in excluded_directory_prefixes
    )
...
def compile_inventory(
    ...
    report = {
        ...
        "semantics": {
            ...
            "historical_material_excluded": True,
            "third_party_environments_excluded": True,
            "semantic_identity_basis": (
                "normalized_python_ast"
            ),
            "same_name_is_not_identity": True,
            "shared_implementation_requires_"
            "authority_preserving_migration": True,
        },
        "counts": {
            ...
            "duplicate_implementation_occurrences": (
...
"""
    ...
    if not semantic_function_digest(
        ellipsis_node
    ):
        raise RuntimeError(
    ...
    if not excluded_directory(
        ".venv-identity-quality"
    ):
        raise RuntimeError(
            "virtual environment exclusion failed"
        )
    ...
    if not excluded_directory(
        ...
        raise RuntimeError(
            "third-party package exclusion failed"
        )
    ...
    if not contains_sequence(
        ...
        raise RuntimeError(
            "historical transaction exclusion failed"
        )
    ...
    return {
        ...
        "constant_normalization": "passed",
        "virtual_environment_exclusion": "passed",
        "historical_material_exclusion": "passed",
