...
schema = "savant.carbon.straub.datrix.v7"
owner = "carbon"
module = "straub"
...
class StraubDatrix:
    ...
    Canonical Datrix substance remains semantic authority.

    Dyad, Umbra, Membrane, and Isotope remain the
    structural primitives.

    Capability modules describe deterministic,
    rebuildable, non-authoritative composition around
    that kernel.

    Service surfaces expose deterministic indexes,
    adapter contracts, transaction plans, and pluggable
    projections. Service state is always derived from the
    current canonical capsule and is never a competing
    semantic authority.

    Isotopes remain deterministic projections of
    ...
    def __init__(
        ...
        *,
        ...
        modules: DatrixModuleSet | None = None,
    ) -> None:
    ...
    def module_manifest(
        ...
        return self._modules.manifest()

    def module_health(
        self,
    ) -> dict[str, Any]:
        return self._modules.health()

    def capabilities(
        self,
    ) -> tuple[str, ...]:
        return (
            self._modules.capabilities()
        )
    ...
    def interfaces(
        ...
        return (
            self._modules.interfaces()
        )

    def module_activation_order(
        self,
    ) -> tuple[str, ...]:
        return (
            self._modules.activation_order()
        )
    ...
    def adapter_registry(
        ...
        return self.services().adapters

    def projection_registry(
        self,
    ) -> DatrixProjectionRegistry:
        return self.services().projections
    ...
    def transaction_plan(
        ...
        return self.services().transaction(
            transaction_id,
            operations,
        )
    ...
    def project_service(
        ...
        return self.project_isotope(
            ...
            provenance={
                "source":
                    "straub.datrix-services",
                "service_projection_type":
                    projection_type,
                "projection_surface":
                    "isotope",
                "semantic_authority":
    ...
    def checkpoint(
        ...
        if not self._modules.compatible():
            raise RuntimeError(
                "datrix module composition "
                "is not compatible"
            )
        ...
        return {
            ...
            "module_set_digest":
                self._modules.fingerprint(),
            "module_count":
                len(
                    self._modules.modules()
                ),
            "services_rebuildable":
                True,
            "services_semantic_authority":
                False,
            "durable":
                True,
    ...
    def component_status(
        ...
        return {
            ...
            "capability_composition":
                "datrix.module-set",
            "capability_composition_authoritative":
                False,
            "service_surface":
                "datrix.service-surface",
            "service_surface_authoritative":
                False,
            "service_surface_rebuildable":
                True,
            "authority_effect":
                "none",
        }
    ...
    def health(
        ...
        status = (
            "ok"
            if (
                module_health["status"] == "ok"
                and service_health["status"] == "ok"
            )
            else "degraded"
        )
        ...
        return {
            ...
            "projection_semantics":
                "deterministic projection",
            "module_semantic_authority":
                False,
            "module_manifest_rebuildable":
                True,
            "service_semantic_authority":
                False,
            "service_surface_rebuildable":
                True,
