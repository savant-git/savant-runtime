...
def _read_catalog() -> Dict[str, Any]:
    try:
        ...
        raise ProviderError(
            f"universal provider catalog missing: {CATALOG_PATH}"
        ) from exc
    except json.JSONDecodeError as exc:
        raise ProviderError(
            f"invalid universal provider catalog: {exc}"
        ) from exc
...
def _request_config(
    ...
    if not isinstance(
        ...
        raise ProviderError(
            f"unknown universal provider profile: {profile_id}"
        )
...
def _request_url(
    ...
    raise ProviderError(
        f"unsupported universal protocol: {protocol}"
    )
...
def _post_json(
    ...
    try:
        ...
        raise ProviderError(
            f"universal provider request failed: {exc}"
        ) from exc
