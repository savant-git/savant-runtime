...
schema = "savant.niche.atlas-server.v4"
...
class AtlasHandler(BaseHTTPRequestHandler):
    ...
    def _dispatch_get(self) -> None:
        ...
        if path == "/api/atlas":
            self._send_json(atlas_projection())
            return

        if path == "/api/atlas/summary":
            self._send_json(summary_projection())
            return

        if path == "/api/atlas/self-check":
            self._send_json(self_check())
            return
        ...
        self._send_json(
            {
                ...
                "mutation_authority": mutation_authority,
                "status": "rejected",
                "error": "Atlas is projection-only",
            },
