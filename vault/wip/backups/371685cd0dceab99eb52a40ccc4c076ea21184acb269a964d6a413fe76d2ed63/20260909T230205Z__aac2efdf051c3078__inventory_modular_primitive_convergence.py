...
def normalized_node(node: ast.AST) -> Any:
    if isinstance(
        node,
        (
            ast.FunctionDef,
        ...
        return {
            ...
            "args": normalized_node(node.args),
            "body": [
            ...
            "returns": normalized_node(node.returns),
    ...
    if isinstance(node, ast.arg):
        return {
            "type": "arg",
            "annotation": normalized_node(node.annotation),
            "type_comment": node.type_comment,
        }
...
def python_paths(root: Path) -> Iterable[Path]:
    for path in sorted(
        root.rglob("*.py")
    ):
        if not path.is_file():
...
def inspect_file(
    ...
    records: list[function_record] = []

    for node, qualified_name in qualified_functions(
        tree
    ):
        size = ast_size(node)
        statement_count = len(node.body)

        if statement_count < minimum_statement_count:
            continue
...
def owner_hint(scope: str) -> dict[str, Any]:
    parts = Path(scope).parts

    if "rubric" in parts:
        index = parts.index("rubric")

        return {
            "classification": "existing_rubric_scope",
            "scope": Path(
                *parts[: index + 1]
            ).as_posix(),
            "authority_effect": "none",
    ...
    if scope.startswith(
        ...
        return {
            "classification": (
                "niche_masterplan_shared_scope"
            ),
            "scope": "tools/niche/masterplan",
            "authority_effect": "none",
        }
...
def disposition(
    ...
    if scope == ".":
        return "investigate_shared_primitive"
...
def compile_inventory(
    ...
    for path in python_paths(root):
        ...
        file_records, failure = inspect_file(
            root,
            path,
        )
    ...
    for record in records:
        ...
        name_digests[
            record.name
        ].add(record.semantic_digest)

    groups: list[
        dict[str, Any]
    ] = []

    for semantic_digest, members in (
        by_digest.items()
    ):
        paths = sorted(
            {
        ...
        group = {
            ...
            "paths": paths,
            "narrowest_common_scope": scope,
            "owner_hint": owner_hint(scope),
            "disposition": disposition(members),
            "authority_effect": "none",
            "members": [
                {
                    "path": member.path,
                    ...
                    "statement_count": (
                        member.statement_count
                    ),
                    "ast_size": member.ast_size,
                }
    ...
    report = {
        ...
        "root": str(root),
        "semantics": {
            "filesystem_presence_is_authority": False,
            "inventory_is_authority": False,
            "automatic_mutation": False,
            "automatic_owner_assignment": False,
            "semantic_identity_basis": (
                "normalized_python_ast"
            ),
            "same_name_is_not_identity": True,
            "shared_implementation_requires"
            "_authority_preserving_migration": True,
        },
        "counts": {
            "python_files_scanned": scanned_files,
            "functions_inspected": len(records),
            "parse_failures": len(
                parse_failures
            ),
            ...
            "files_participating_in"
            "_duplicate_implementations": (
                len(duplicate_files)
            ),
            "same_name_different"
            "_substance": len(
...
"""
    ...
    first_digest = (
        semantic_function_digest(first)
    )

    second_digest = (
        semantic_function_digest(second)
    )

    third_digest = (
        semantic_function_digest(third)
    )
    ...
    if scope != "tools/niche/masterplan":
        raise RuntimeError(
            "common scope failed"
        )
    ...
    if digest(
        ...
        raise RuntimeError(
            "deterministic fingerprint failed"
        )
