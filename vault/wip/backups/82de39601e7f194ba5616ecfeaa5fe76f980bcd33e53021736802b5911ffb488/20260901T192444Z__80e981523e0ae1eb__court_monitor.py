...
def record_snapshot(db: sqlite3.Connection, kind: str, payload: Any) -> tuple[bool, bool, str, Any]:
    ...
    return initial, (before is not None and before[0] != sha), sha, clean


def require_case_info(response: dict[str, Any], expected_case: str) -> None:
    info = response.get("CaseInfo") or response.get("caseInfo")
    if not isinstance(info, dict) or not info:
        raise RuntimeError(
            "Clerk API returned success but no CaseInfo. Verify the local case "
            f"number ({expected_case}) and account access."
        )


def require_docket_container(response: dict[str, Any]) -> None:
    # An empty docket list is legitimate; a response with no docket container
    # at all usually means the API returned a status envelope without data.
    if not any(k in response for k in ("DocketsList", "docketsList", "DocketList", "dockets")):
        raise RuntimeError(
            "Clerk API returned success but no docket data container. The case "
            "may not be available to this API account yet."
        )
...
def check(cfg: dict[str, Any], key: str) -> int:
    ...
    case_current = root / "case-information" / "current"
    case_snapshots = root / "case-information" / "snapshots"
    docket_current = root / "dockets" / "current"
    docket_snapshots = root / "dockets" / "snapshots"
    docs = root / "documents" / "originals"
    quarantine = root / "documents" / "quarantine"
    events_dir = root / "events"
    database_dir = root / "database"
    exports_dir = root / "exports"
    for directory in (case_current, case_snapshots, docket_current, docket_snapshots,
                      docs, quarantine, events_dir, database_dir, exports_dir):
        directory.mkdir(parents=True, exist_ok=True)
    os.chmod(root, 0o700)
    db = connect(database_dir / "history.sqlite3")
    ...
    try:
        ...
        api_case = str(cfg.get("api_case_number") or re.sub(r"[^A-Za-z0-9]", "", case))
        if not api_case:
            raise RuntimeError("API case number is empty after normalization")
        info = client.get({"caseNumber": api_case})
        require_case_info(info, case)
        info_initial, info_changed, info_sha, clean_info = record_snapshot(db, "case_info", info)
        if info_initial: changes.append("initial case snapshot created")
        if info_changed: changes.append("case information changed")
        dockets_raw = client.get({"civilCaseNumber": api_case})
        require_docket_container(dockets_raw)
        docket_initial, docket_changed, docket_sha, clean_dockets = record_snapshot(db, "docket_response", dockets_raw)
        if docket_initial: changes.append("initial docket snapshot created")
        if docket_changed: changes.append("docket changed")
        ...
        if cfg.get("download_documents"):
            ...
            for number in missing_numbers:
                ...
                try:
                    response = client.get({"caseNumber": api_case, "docketNumber": number})
                    blob = decode_document(response); sha = hashlib.sha256(blob).hexdigest()
                    year = str(dt.datetime.now(UTC).year)
                    year_dir = docs / year
                    year_dir.mkdir(exist_ok=True)
                    path = year_dir / f"docket-{int(number):05d}-{sha[:12]}{extension(blob)}"
        ...
        atomic_json(case_current / "latest-case.json", clean_info)
        atomic_json(docket_current / "latest-dockets.json", clean_dockets)
        stamp = dt.datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
        if info_initial or info_changed or not any(case_snapshots.iterdir()):
            atomic_json(case_snapshots / f"{stamp}-{info_sha[:12]}.json", clean_info)
        if docket_initial or docket_changed or not any(docket_snapshots.iterdir()):
            atomic_json(docket_snapshots / f"{stamp}-{docket_sha[:12]}.json", clean_dockets)
        ...
        if changes:
            atomic_json(events_dir / f"{stamp}-change.json", {
                "case_number": case, "detected_at": now(), "changes": changes,
                "requests": client.requests, "units_balance": balance
            })
...
def status(cfg: dict[str, Any]) -> int:
    db_path = Path(os.path.expanduser(cfg["data_dir"])) / "database" / "history.sqlite3"
...
def main() -> int:
    ...
    p.add_argument("--config", default="~/clerk/sys/court-monitor.json")
    args = p.parse_args()
