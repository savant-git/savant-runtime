...
schema = (
    ...
    "envoy-runtime-package-compat/1.1.0"
)
...
def _load_package_module(
    ...
    if (
        ...
        raise envoy_runtime_package_compat_error(
            "unable to construct Envoy "
            f"package import: {path}"
        )
...
def install() -> dict[str, Any]:
    ...
    for synthetic_name, bare_name in (
        synthetic_aliases.items()
    ):
        _bind_synthetic_alias(
            synthetic_name,
            bare_name,
        )
