...
(() => {
  ...
  const shapes=["rect","wide","tall","l-right","l-left","step-right","step-left","u","offset","inverted-l","t-bay","dual-wing","corner-cut","double-chamfer","spine-wing"];
  ...
  function shapeFor(t,i) {
    ...
    if(objectives>=5&&count>=18) return seed%2?"dual-wing":"spine-wing";
    if(crossings>=10&&count>=14) return seed%2?"t-bay":"double-chamfer";
    if(objectives>=4&&count>=12) return seed%3===0?"inverted-l":seed%2?"l-left":"l-right";
    if(count>=30) return seed%2?"offset":"u";
    if(count>=20) return seed%2?"step-left":"step-right";
    if(count>=12) return seed%3===0?"corner-cut":"wide";
    if(count>=8&&crossings>=4) return "tall";
    ...
    s.innerHTML=`
      <div class="atlas-shell">
        <header class="atlas-header">
          ...
          <div class="atlas-search-wrap"><input id="atlas-search" class="atlas-input" type="search" autocomplete="off" placeholder="Search tasks, objectives, territories…" aria-label="Search Atlas"><button id="atlas-rail-toggle" class="atlas-rail-toggle" type="button" aria-expanded="false" aria-controls="atlas-rail">CONTROLS</button></div>
        </header>
        <div class="atlas-main">
          <section class="atlas-map-column">
            ...
            <div class="atlas-selected-strip">
              ...
              <div class="atlas-selected-actions">
                ...
                <button id="atlas-frontier" type="button">FRONTIER</button><button id="atlas-back" type="button" disabled>BACK</button><button id="atlas-forward" type="button" disabled>FORWARD</button>
              </div>
            </div>
          </section>
          <aside class="atlas-rail" id="atlas-rail">
            ...
            <section class="atlas-panel atlas-detail-panel"><div class="atlas-panel-heading"><h2>LOCATION DETAIL</h2></div><div id="atlas-detail" class="atlas-detail"><p>Select a territory, port, or task.</p></div></section><section class="atlas-panel"><div class="atlas-panel-heading"><h2>ROUTE</h2></div><input id="atlas-route-input" class="atlas-input" type="search" placeholder="Destination task…"><button id="atlas-route-go" class="atlas-wide" type="button">FIND REPRESENTED ROUTE</button><div id="atlas-route" class="atlas-route"></div></section>
  ...
  function renderTerritories(ts) {
    ...
    for(const t of ts){
      ...
      g.append(svg("polygon",{points:pstr(p.points),class:"atlas-territory-outer"}),svg("polygon",{points:pstr(inner),class:"atlas-territory-inner"}));renderSubzones(g,t,p);g.append(svg("line",{x1:p.x+22,y1:p.y+G.head-14,x2:p.x+Math.min(p.w-22,410),y2:p.y+G.head-14,class:"atlas-territory-header","vector-effect":"non-scaling-stroke"}),title,meta,svg("circle",{cx:p.x+p.w-34,cy:p.y+32,r:17,class:"atlas-territory-count-circle"}),count);
  ...
  function renderAll(){renderState();if(!state.ready)return;renderMetrics();renderSelected();renderBreadcrumb();renderDetail();renderRoute();renderLegend();if(state.size.w>720||state.railOpen)renderIndex();else dom.index.replaceChildren();renderMap();renderMinimap();dom.footerStatus.textContent=`ATLAS ${state.level.toUpperCase()}`}
  ...
  function bindMap() {
    dom.viewport.addEventListener("pointerover",e=>{const target=e.target.closest?.("[data-task-id],[data-territory-id]");if(target)showTooltip(e,target)});
    dom.viewport.addEventListener("pointerout",e=>{if(e.target.closest?.("[data-task-id],[data-territory-id]"))hideTooltip()});
    dom.viewport.addEventListener("pointerdown",hideTooltip,{capture:true});
    dom.viewport.addEventListener("wheel",e=>{e.preventDefault();zoomAt(Math.exp(-e.deltaY*.00125),e.clientX,e.clientY)},{passive:false});
    dom.viewport.addEventListener("click",e=>{const t=e.target.closest?.("[data-task-id]");if(t?.dataset.taskId)return select(t.dataset.taskId,false,"map");const port=e.target.closest?.("[data-port-direction]");if(port){const source=state.semantic.territoryById.get(port.dataset.edgeSource),target=state.semantic.territoryById.get(port.dataset.edgeTarget);state.detail={type:"port",direction:port.dataset.portDirection,count:Number(port.dataset.count)||0,sourceLabel:source?.label??port.dataset.edgeSource,targetLabel:target?.label??port.dataset.edgeTarget};queueRender();return}const x=e.target.closest?.("[data-territory-id]");if(x?.dataset.territoryId&&state.semantic.territoryById.has(x.dataset.territoryId)){state.focusTerritory=x.dataset.territoryId;state.selected=null;state.detail=null;fitTerritory(x.dataset.territoryId);queueRender()}});
  ...
  function setRailOpen(open) {
    ...
    document.documentElement.classList.toggle("atlas-rail-open",state.railOpen);if(state.railOpen)queueRender();
  }

  function bindControls() {
    dom.nav?.addEventListener("click",open);
    dom.railToggle?.addEventListener("click",()=>setRailOpen(!state.railOpen));dom.fit.onclick=fitWorld;dom.local.onclick=()=>state.selected?focusTask(state.selected):state.focusTerritory?fitTerritory(state.focusTerritory):fitWorld();
    dom.plus.onclick=()=>zoomCenter(Z.step);dom.minus.onclick=()=>zoomCenter(1/Z.step);dom.back.onclick=goBack;dom.forward.onclick=goForward;dom.frontier.onclick=frontier;dom.focusTask.onclick=()=>state.selected&&focusTask(state.selected);dom.openTask.onclick=()=>state.selected&&window.Niche?.task?.(state.selected);
  ...
  function restore() {
    const x=sread();if(!x)return;
    if(x.viewport&&Number.isFinite(x.viewport.x)&&Number.isFinite(x.viewport.y)&&Number.isFinite(x.viewport.scale)){state.viewport={x:x.viewport.x,y:x.viewport.y,scale:clamp(x.viewport.scale,Z.min,Z.max)};state.level=levelFor(state.viewport.scale)}
    state.filterTouched=Boolean(x.filterTouched);
    if(state.filterTouched){
      state.query=txt(x.query);
      state.status=["all","ready","active","review","waiting","blocked","complete","unknown"].includes(x.status)?x.status:"all";
      state.priority=["all","critical","high","medium","low","unknown"].includes(x.priority)?x.priority:"all";
      state.completed=["show","fade","hide"].includes(x.completed)?x.completed:"show";
    }else{
      state.query="";state.status="all";state.priority="all";state.completed="show";
    }
    ...
    state.focusTerritory=txt(x.focusTerritory)||null;
  }
  function applyPrefs(){dom.search.value=state.query;dom.status.value=state.status;dom.priority.value=state.priority;dom.completed.value=state.completed;dom.effects.value=state.effects;for(const b of $$("[data-lens]",dom.lenses))b.classList.toggle("active",b.dataset.lens===state.lens)}
  function bindCore(){
    for(const n of ["niche:projection","niche:task-selected"])window.addEventListener(n,sync);window.addEventListener("niche:theme-will-change",hideTooltip);window.addEventListener("niche:theme-change",()=>{if(state.installed){queueRender();queueTransform();emit("theme",{theme:document.documentElement.dataset.nicheTheme})}});
