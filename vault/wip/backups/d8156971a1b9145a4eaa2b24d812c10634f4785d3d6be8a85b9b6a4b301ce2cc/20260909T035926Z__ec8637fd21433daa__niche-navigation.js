...
(() => {
  ...
  const BUILD_VERSION = "atlas-r10.1-facility-verified";
  ...
  const emit = (type,detail={}) => window.dispatchEvent(new CustomEvent(`niche:atlas:${type}`,{detail:{...detail,build:BUILD_VERSION,authority_effect:"none",projection_only:true}}));
  ...
  function pack(territories) {
    ...
    const baseW=Math.max(1180,Math.sqrt(area*aspect)*1.015);
    const candidates=[];
    for(const multiplier of [1,1.08,1.16,1.26]){
      const targetW=Math.ceil(baseW*multiplier);
      const targetH=Math.ceil(Math.max(820,area/targetW*1.34));
      ...
      <div class="atlas-shell">
        <header class="atlas-header">
          <div class="atlas-brand">
            <span>SAVANT</span>
            <h1>ATLAS</h1>
            <span class="atlas-build-tag" style="font-size:0.52rem;color:var(--a-accent);opacity:0.85;letter-spacing:0.18em;border:1px solid currentColor;padding:2px 4px;border-radius:2px;">R10.1 FACILITY VERIFIED</span>
          </div>
