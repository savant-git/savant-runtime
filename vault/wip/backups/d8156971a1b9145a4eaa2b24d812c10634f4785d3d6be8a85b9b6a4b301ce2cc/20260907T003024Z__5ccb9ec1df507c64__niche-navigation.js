...
(() => {
  ...
  const state = {
    ...
    completed:"fade", effects:"full", railOpen:false, route:[], back:[], forward:[], recent:[], detail:null,
  ...
  function renderAll(){renderState();if(!state.ready)return;renderMetrics();renderSelected();renderBreadcrumb();renderDetail();renderRoute();renderLegend();renderIndex();renderMap();renderMinimap();dom.footerStatus.textContent=`ATLAS ${state.level.toUpperCase()}`}
  ...
  function bindControls() {
    ...
    dom.status.onchange=()=>{state.status=dom.status.value;swrite();queueRender()};dom.priority.onchange=()=>{state.priority=dom.priority.value;swrite();queueRender()};dom.completed.onchange=()=>{state.completed=dom.completed.value;swrite();queueRender()};dom.effects.onchange=()=>{state.effects=dom.effects.value;swrite();queueRender()};
    dom.reset.onclick=()=>{state.query="";state.status="all";state.priority="all";state.completed="fade";state.focusTerritory=null;dom.search.value="";dom.status.value="all";dom.priority.value="all";dom.completed.value="fade";swrite();queueRender()};
  ...
  function restore() {
    ...
    state.query=txt(x.query);state.status=txt(x.status,"all");state.priority=txt(x.priority,"all");state.lens=txt(x.lens,"domains");state.completed=txt(x.completed,"fade");state.effects=txt(x.effects,"full");state.focusTerritory=txt(x.focusTerritory)||null;
  }
  ...
  function bindCore(){
    for(const n of ["niche:projection","niche:task-selected"])window.addEventListener(n,sync);
