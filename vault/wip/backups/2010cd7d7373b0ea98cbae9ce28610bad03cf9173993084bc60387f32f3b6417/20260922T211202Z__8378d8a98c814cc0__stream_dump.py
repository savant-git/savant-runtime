...
def freeze_candidate(
    ...
    try:
        ...
        raise StreamDumpError(f"unable to freeze source {relative}: {error}") from error
    ...
    if (
        ...
        raise StreamDumpError(f"source changed while snapshotting: {relative}")
...
def stream_dump(target_raw: str) -> tuple[Path, list[Any], dict[str, Any]]:
    ...
    for candidate in candidates:
        frozen.append(
            freeze_candidate(candidate, snapshot_root, datrix_index)
        )

    frozen_candidates = [item[0] for item in frozen]
    candidate_paths = {item.relative_path for item in candidates}
    frozen_paths = {item.relative_path for item in frozen_candidates}

    if candidate_paths != frozen_paths:
        raise StreamDumpError("source admission and frozen snapshot sets differ")

    frozen_bytes = sum(item[2].st_size for item in frozen)
    if frozen_bytes != discovery["candidate_bytes"]:
        raise StreamDumpError("frozen source bytes differ from admission")
    ...
    with gzip.open(
        ...
        rendered_bytes += write_json_line(
            handle,
            {
                ...
                "purpose": (
                    "portable comprehensive Savant program-source and "
                    "architecture-context evidence sufficient to understand, "
                    "continue, reconstruct, and evolve the admitted implementation"
                ),
                "source_admission": "broad-positive-source-with-concrete-junk-exclusions",
                "source_snapshot": "immutable-transaction-local",
                "source_files": len(frozen),
        ...
        for candidate, record, file_stat, actual_sha, _matches in frozen:
            ...
            rendered_bytes += write_file_record(
                handle, candidate, record, file_stat, actual_sha, artifact_digest
            )
        ...
        rendered_bytes += write_json_line(
            handle,
            {
                "admitted_files": len(candidate_paths),
                ...
                "admission_equals_frozen": candidate_paths == frozen_paths,
                "admission_equals_serialized": candidate_paths == serialized_paths,
                "bytes_equal": included_source_bytes == discovery["candidate_bytes"],
                "complete": (
                    candidate_paths == frozen_paths
                    and candidate_paths == serialized_paths
    ...
    if (
        candidate_paths != serialized_paths
        or included_source_bytes != discovery["candidate_bytes"]
    ):
        raise StreamDumpError("strict source completeness invariant failed")

    return output, frozen_candidates, {
        "target": target,
        "discovery": discovery,
...
def copy_authoritative_source(checkout: Path, candidates: Iterable[Any]) -> None:
    for candidate in candidates:
        destination = checkout / candidate.relative_path
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(candidate.absolute_path, destination)
...
def push_source_to_github(
    ...
    with tempfile.TemporaryDirectory(prefix="savant-sdump-git-") as temporary:
        ...
        copy_authoritative_source(checkout, candidates)
...
def delete_local_artifact(artifact: Path, snapshot_root: Path) -> None:
...
def publish(
    ...
    github_result = push_source_to_github(candidates, environment)
    delete_local_artifact(artifact, snapshot_root)
