...
def validate_atlas_block(
    ...
    frontend_location = re.search(
        r"location\s+/atlas/\s*\{"
        r"(?P<body>.*?)"
        r"\}",
        block,
        re.DOTALL,
    ...
    if frontend_location is None:
        raise MigrationError(
            "Atlas frontend location unavailable"
        )

    frontend_body = (
        frontend_location.group(
            "body"
        )
    )
    ...
    if (
        "proxy_pass http://127.0.0.1:8766/;"
        not in frontend_body
    ):
        raise MigrationError(
            "Atlas frontend proxy does not strip "
            "the /atlas/ prefix"
        )
