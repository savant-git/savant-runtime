...
schema = "savant.sdump.stream.v9"
command_schema = "savant.sdump.command.v9"
...
class TerminalUI:
    ...
    def phase(
        ...
        suffix = (
            f"  {self.c(detail, self.dim)}"
            if detail
            else ""
        )
        self.line(
            f"{self.c(marker, self.blue)} "
            f"{self.c(name, self.bold)}"
            f"{suffix}"
        )
...
def safe_relative(
    ...
    try:
        ...
        raise StreamDumpError(
            f"path escaped target: {path}"
        ) from error


def is_hidden_relative(
    relative: str,
) -> bool:
    parts = Path(
        relative
...
def is_structured_source(
    ...
    parent_parts = tuple(
        part.casefold()
        for part
        in Path(relative).parts[:-1]
    )

    if (
        name in structured_source_names
        or name in text_source_names
    ):
        return True
    ...
    if suffix in {
        ...
        return any(
            part in structured_source_dirs
...
def admitted(
    ...
    if (
        relative
        in excluded_exact_relative_paths
    ):
        return False
    ...
    for (
        ...
        for name in files:
            ...
            try:
                ...
                raise StreamDumpError(
                    "unable to stat source candidate "
                    f"{path}: {error}"
                ) from error
...
def stream_dump(
    ...
    if not target.is_dir():
        raise StreamDumpError(
            "sdump target must be a directory"
        )
    ...
    ui.phase(
        "discover",
        (
            "pruning dependencies, generated "
            "state, recovery, and historical bulk"
        ),
    )
    ...
    if not candidates:
        raise StreamDumpError(
            "no relevant Savant source admitted"
        )
    ...
    if (
        ...
        ui.note(
            "admission warning",
            (
                "source corpus exceeds 512 MiB; "
                "generated material may still "
                "be admitted"
            ),
            "warn",
        )
            ...
            write_json_line(
                handle,
                {
                    ...
                    "purpose": (
                        ...
                        "current Savant source "
                        "and architecture context"
                    ),
                    "source_admission": (
                        "current-source-positive-"
                        "with-generated-dependency-"
                        "history-exclusions"
                    ),
                    "source_files": len(
                        candidates
...
def push_source_to_github(
    ...
    if oversized:
        raise StreamDumpError(
            "GitHub corpus contains "
            "file(s) at or above 100 MiB: "
            + ", ".join(
                oversized[:5]
            )
        )
...
def publish(
    ...
    ui.note(
        "s3 verified",
        (
            f"{ui.human_bytes(s3_result['bytes'])}  "
            f"sha256 {artifact_sha256[:12]}…  "
