...
def discover(
    ...
    profile: Profile,
    absolute_excluded_roots: Iterable[
        Path
    ],
    include_hidden: bool,
    ...
    for root in targets:
        ...
        if root.is_file():
            try:
                ...
                if not _authoritative_candidate(
                    root,
                    profile,
                    st.st_mode,
                ):
...
def build_projection(
    ...
    canonical_by_source_hash: dict[
        str,
        str,
    ] = {}

    admitted_by_source_hash: dict[
        str,
        bool,
    ] = {}
    ...
    stats.included_files = len(
        included
    )

    stats.unique_contents = sum(
        record.duplicate_of
        is None
    ...
    stats.budget_bytes_limit = (
        profile
        .max_total_content_bytes
    )

    stats.budget_bytes_used = (
        unique_rendered_bytes
    )

    stats.budget_bytes_remaining = max(
        0,
        (
            profile
            .max_total_content_bytes
