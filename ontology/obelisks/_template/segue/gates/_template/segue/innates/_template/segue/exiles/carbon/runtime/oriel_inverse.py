...
class SearchPolicy:
    ...
    maximum_candidate_shift_minutes: int = 1440
    maximum_existing_shift_minutes: int = 720
    maximum_depth: int = 3
    maximum_states: int = 500
    solution_limit: int = 8
    candidate_shift_cost: float = 1.0
    existing_shift_cost: float = 4.0
    location_change_cost: float = 240.0
    mode_change_cost: float = 180.0
    edit_cost: float = 25.0
    conditional_penalty: float = 500.0
    unresolved_penalty: float = 1000.0
    earlier_change_bonus: float = 0.0
    ...
    def _existing_event_neighbors(
        ...
        for identifier in blocking_ids:
            ...
            delta = (
                ...
                else policy.resolution_minutes
            )
    ...
    def search(
        ...
        *,
        ...
        search_policy = (
            ...
            else SearchPolicy.from_mapping(
                policy
            )
        )
        ...
        "capabilities": [
            {
                ...
                "purpose":
                    (
                        "Search backward from a "
                        "desired event for the smallest "
                        "admissible spacetime changes "
