...
(() => {
    ...
    async function refresh({ quiet = false } = {}) {
        ...
        try {
            const results = await Promise.allSettled([
                ...
                fetchJson(API.dashboard, signal),
                fetchJson(API.state, signal),
                fetchJson(API.tasks, signal),
                fetchJson(API.history, signal),
                fetchJson(API.living, signal),
                fetchJson(API.fabric, signal)
            ]);
            ...
            if (tasks.status === "fulfilled") {
                const nextTasks = extractTasks(tasks.value);
                const nextDigest = digestTasks(nextTasks);
            ...
            if (living.status === "fulfilled") {
                state.living = living.value;
            }

            if (fabric.status === "fulfilled") {
                state.fabric = fabric.value;
            }
            ...
            if (!quiet) {
                toast("Niche projections refreshed.");
            }
        } catch (error) {
    ...
    function renderStatus() {
        setStatus(
            ...
            state.tasks.length
                ? `${state.tasks.length} projected`
                : "unknown"
        );

        const fabric = object(state.fabric);
        const fabricHealthy = bool(
            first(
                fabric.healthy,
    ...
    function projectionSnapshot() {
        return Object.freeze({
            ...
            requestGeneration: state.projectionGeneration,
