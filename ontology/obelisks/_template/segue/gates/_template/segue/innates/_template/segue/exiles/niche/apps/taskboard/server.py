...
from mentor_subsystem import MentorSubsystem
...
mentor_subsystem = MentorSubsystem(app_root)
...
mentor_palaver_url = "http://127.0.0.1:8787/api/chat"
mentor_max_context_chars = 14000
mentor_timeout_seconds = 90
...
class TaskboardHandler(
    ...
    def _get(
        ...
        if path == "/api/mentor/document":
            self.send_json(
                mentor_subsystem.document()
            )
            return

        if path == "/api/mentor/status":
            self.send_json(
                mentor_subsystem.refresh()
            )
            return

        if path.startswith("/api/mentor/source/"):
            kind = path[
                len("/api/mentor/source/"):
            ]
            self.send_json(
                mentor_subsystem.source_document(
                    kind
                )
            )
            return
    ...
    def _post(
        ...
        if path == "/api/mentor/chat":
            ...
            section_context = str(
                ...
                or ""
            )[:mentor_max_context_chars]

            mentor_prompt = (
                "You are operating as Mentor inside Savant Niche. "
                "Use the supplied masterplan/structure projection context "
                "as evidence, not independent authority. Preserve Savant "
                "authority precedence. Distinguish accepted authority, "
                "verified implementation, projection, inference, and "
                "unknown. Do not mutate authority or claim completion "
                "without evidence.\n\n"
                f"document: {document}\n"
                f"revision: {revision or 'unknown'}\n"
                f"section_id: {section_id or 'whole-document'}\n"
                f"section_title: {section_title or 'whole document'}\n\n"
                "selected context:\n"
                f"{section_context or '[no section selected]'}\n\n"
                "user question:\n"
                f"{question}"
            )
            ...
            try:
                with urlopen(
                    request,
                    timeout=mentor_timeout_seconds,
                ) as response:
                    palaver = json.loads(
                ...
                self.send_json(
                    {
                        "ok": False,
                        "error":
                            "Palaver rejected the Mentor request.",
                ...
                self.send_json(
                    {
                        "ok": False,
                        "error":
                            "Mentor orchestration is unavailable.",
                        "diagnostic": str(exc),
                        "route":
                            "niche → palaver → opus",
                        "authority_effect":
                ...
                self.send_json(
                    {
                        "ok": False,
                        "error":
                            "Palaver returned an invalid Mentor response.",
                        "route":
                            "niche → palaver → opus",
                        "authority_effect":
                            "none",
