...
for CANDIDATE in \
  "$ROOT/palaver_ultra" \
  "$ROOT/webui_ultra" \
  "$ROOT/ontology/obelisks/_template/segue/gates/_template/segue/innates/_template/segue/exiles/palaver/apps/webui-nextgen"
do
  if [ -d "$CANDIDATE" ]; then
    APP="$CANDIDATE"
    break
...
if [ -z "$APP" ]; then
  echo "[ERROR] Could not find Palaver frontend app."
  exit 1
fi

echo "[OK] Using app: $APP"

mkdir -p "$APP/src"

cat > "$APP/src/PalaverVoice.jsx" <<'JSX'
...
import PalaverVoice from "./PalaverVoice.jsx";
...
echo
echo "[OK] Palaver voice UI installed."
echo
echo "Start preview:"
echo "cd \"$APP\""
...
echo
echo "NOTE:"
echo "Browser microphone requires HTTPS or localhost."
echo "If using a raw server IP over http, voice recognition may be blocked."
